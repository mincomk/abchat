...
use sqlx::{PgPool, Row};
use crate::{AppResult, Message, User, ServiceError, AppError};
...
impl Persistence for PostgresPersistence {
    ...
    async fn list_users(&self) -> AppResult<Vec<User>> {
        let rows = sqlx::query(
            "SELECT username, nickname, is_admin FROM users"
        )
        .fetch_all(&self.pool)
        .await
            .map_err(|e| AppError::Service(ServiceError::Database(e)))?;
    ...
    async fn get_user(&self, username: &str) -> AppResult<Option<User>> {
        let row = sqlx::query(
            "SELECT username, nickname, is_admin FROM users WHERE username = $1"
        )
        .bind(username)
    ...
    async fn list_messages(
        ...
        let rows = sqlx::query(
            "SELECT id, content, timestamp, channel_id FROM messages WHERE channel_id = $1 LIMIT $2 OFFSET $3"
        )
        .bind(channel_id)
        .bind(limit as i64)
        .bind(offset as i64)
